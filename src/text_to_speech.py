...
def synthesize_english_speech(text, output_filename, voice_id="Matthew"):
    """
    英語テキストを音声に変換（男性声）
    """
    logger.info("英語音声合成開始")
    ...
    try:
        ...
        if "AudioStream" in response:
            ...
            logger.info(f"英語音声合成完了: {output_filename}")
            return output_filename
        else:
            logger.error("AudioStreamがレスポンスに含まれていません")
            return None
    except Exception as e:
        logger.error(f"英語音声合成中にエラー: {str(e)}")
        return None
...
def generate_audio_for_article(article):
    ...
    try:
        ...
        # 英語音声生成
        english_output_filename = f"{AUDIO_DIR}/{article['id']}_en.mp3"
        english_audio = synthesize_english_speech(
        ...
        if english_audio and japanese_audio:
            if IS_LAMBDA:
                # Lambda環境では後でS3にアップロードするための印をつける
                article["english_audio_file"] = english_audio
                article["japanese_audio_file"] = japanese_audio
            else:
                # ローカル環境ではファイルパスをそのまま使用
                article["english_audio_url"] = english_audio
