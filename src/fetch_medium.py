...
import os
import logging
from src.config import IS_LAMBDA, AUDIO_DIR
...
def fetch_medium_articles():
    """
    Mediumのプログラミングタグのフィードを取得
    """
    feed_url = "https://medium.com/feed/tag/programming"
    logger.info(f"Mediumフィード取得開始: {feed_url}")
    ...
    try:
        ...
        logger.error(f"Mediumフィード取得中にエラー: {str(e)}")

    return articles


def save_articles_for_testing(articles, filename="medium_articles.json"):
    """
